...
function GroupItem({ group, color }: { group: any, color: any }) {
  ...
  return (
    ...
    >
      {/* Icon */}
      {group.avatar_url ? (
        <img src={resolvedAvatar} className="h-[54px] w-[54px] shrink-0 rounded-[18px] object-cover shadow-sm" alt="group avatar" />
      ) : (
      ...
      <div className="mx-5 mt-8">
        ...
        <div className="flex flex-col gap-3">
          {filteredGroups?.map((group: any, idx: number) => {
            ...
            return <GroupItem key={group.id} group={group} color={color} />;
          })}
